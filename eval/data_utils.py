...
def save_full_sent_data(outfile, path="../boxscore-data/rotowire", multilabel_train=False, nonedenom=0):
    ...
    with open(outfile.split('.')[0] + ".dict", "w+", encoding="utf-8") as f:
        for i in range(1, len(revvocab)+1):
            f.write("%s %d \n" % (revvocab[i], i))

    with open(outfile.split('.')[0] + ".labels", "w+", encoding="utf-8") as f:
...
def prep_generated_data(genfile, dict_pfx, outfile, path="../boxscore-data/rotowire", test=False):
    ...
    vocab = {}
    with open(dict_pfx+".dict", "r", encoding="utf-8") as f:
        for line in f:
            pieces = line.strip().split()
            vocab[pieces[0]] = int(pieces[1])

    labeldict = {}
    with open(dict_pfx+".labels", "r", encoding="utf-8") as f:
        for line in f:
            pieces = line.strip().split()
    ...
    with open(os.path.join(path, valfi), "r", encoding="utf-8") as f:
        valdata = json.load(f)
