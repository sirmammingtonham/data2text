...
ct = 0
scores_ct = 0
expected_scores_ct = 0
new_p = ''
in_sentence_flag = False
found_score = False
current_word = ''
...
with open('bart_valid.txt', 'r') as f:
	for paragraph in f:
		...
		while len(new_p) != len(paragraph):
			for sentence in paragraph.split('.'):
				# if total_name_list[ct][0] in sentence:
				# 	print(total_name_list[ct][0])
				print(len(paragraph), len(new_p))

				if total_score_gen_list[ct][0] in sentence and total_name_list[ct][0] in sentence:
					for word in sentence.split(' '):
						if word == total_score_gen_list[ct][0]:
							print(total_score_list[ct][0])
							
							new_p += total_score_list[ct][0]
							new_p += ' '
